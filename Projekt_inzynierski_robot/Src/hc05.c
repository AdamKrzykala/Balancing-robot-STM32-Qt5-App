...
int HC05_Parse_Data_frame(struct Data_frame_from_PC *_data, uint8_t *_frame) {
	...
	/* Filters data */
	_data->Complementary_filter_weight = HC05_Merge_bytes(_frame[13], _frame[14]);
	_data->Kalman_filter_process_variance = HC05_Merge_bytes(_frame[15], _frame[16]);
	_data->Madgwick_filter_beta = HC05_Merge_bytes(_frame[17], _frame[18]);

	/* Engines data */
	_data->Left_engine_speed  = HC05_Merge_bytes(_frame[19], _frame[20]);
	_data->Right_engine_speed = HC05_Merge_bytes(_frame[21], _frame[22]);

	/* Additional data */
	_data->Emergency_stop = _frame[23];
	_data->Which_filter = _frame[24];

	/* CRC test */
	Received_CRC = _frame[25];
